...
interface EnergySolutionButtonsProps {
  ...
  // onSelectionChange?: (active: string[]) => void; //wawa
  onSelectionChange?: (active: string[], finalRating?: string | null) => void;
}

export const EnergySolutionButtons: React.FC<EnergySolutionButtonsProps> = ({ showHeader, isExpanded, onExpand, onSelectSolution, buildingData, showYellowBox = true, onToggleYellowBox, yearlyConsumption = '', onProcessClick, onCloseYellowBox, onTotalSavingsChange, onSelectionChange }) => {
  ...
  // }, [checkedItems, yearlyConsumption, buildingData, onTotalSavingsChange]);
      ...
      if (onSelectionChange) {
        ...
        const active = Array.from(checkedItems).map(
          (idx) => ENERGY_SOLUTIONS[idx]
        );
        const finalRating = newRating || estimatedRating || null;
        // onSelectionChange(active);
        // onSelectionChange(active, finalRating);
        if (onSelectionChange) onSelectionChange(active, finalRating);
      }
    }, [
      checkedItems,
      yearlyConsumption,
