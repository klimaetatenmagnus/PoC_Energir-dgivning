...
- Synk `content/` til staging-bucket og valider `/api/solar/solinnstraling` med et bygg der takflater finnes (bruk adresse-lookup for å hente `bygg_id`/koordinater). (Neste agent)
- Kjør Cloud Build smoke-/integrasjonstester mot staging (`npm run test:smoke -- --baseUrl=https://energinokkelen-168751968131.europe-north1.run.app`) når gyldige testdata er avklart. (Neste agent)
