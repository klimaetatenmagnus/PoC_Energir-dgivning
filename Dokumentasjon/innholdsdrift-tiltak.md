...
1. **Etablere admin-Cloud Run + IAP (NY):** Sett opp `energinokkelen-admin`/`energinokkelen-admin-prod` som egne Cloud Run-tjenester med `admin-api`-container, opprett `staging-admin-neg`/`prod-admin-neg`, knytt dem til nye backend services med `/admin`-path-regler i load balanceren, aktiver IAP med OAuth-klienten `Energinokkelen-IAP` og tildel `group:energinokkelredaktor@klimaoslo.no` rollen `roles/iap.httpsResourceAccessor`, og fjern `allUsers` fra admin-tjenestene når flyten er verifisert.
2. **Design / Punkt-polish (PÅGÅR 2025-11-20→):** Kvalitetssikre mobilvisninger etter breakpoint-utrulling og finjustere spacing/typografi der admin-UI avviker fra Punkt-manualen.
3. **IAM og tilgangsstyring (GJENSTÅR):** Admin-UI må låses bak IAP/IAM før staging/prod-bruk. Dette innebærer justering av `EnvironmentToggle`-tilganger, Cloud Run/IAP-konfig og test av `X-Goog-Authenticated-User-Email`-flyten beskrevet i § 9.7.
4. **Redigerbare tiltak/tilskudd (GJENSTÅR):** UI-et er foreløpig “read only” utover fordelseditoren. Vi må bygge skjemaer for tiltak/tilskudd, koble dem til Admin-API for skriving til `content/tiltak/*.json` og `content/tilskudd/*.json`, sørge for schema-validering i UI, samt støtte versjonering/generation-sjekker mot GCS. Dette er siste store funksjon før redaktørene kan jobbe uten Git.
5. **Tilskudds-workflow og paginering (GJENSTÅR):** Når katalogen vokser må vi legge til server-side filtrering/paginering og eventuelt splitting av tilskudd vs tiltak i egne dashboards. Dette henger sammen med punkt 3 slik at redigeringsopplevelsen skalerer.
6. **Figma/Tiltak-komponentopprydding (GJENSTÅR):** Tiltakskomponentene under `src/components/FigmaBlokk/**` har fremdeles hardkodede attributter som gir React-warnings. Disse må ryddes i takt med design-polish slik at forhåndsvisningen speiler produksjonskortene uten konsollstøy.

> Status 2025-11-19: Punkt-prototypen og de fleste leseflyter er på plass (forhåndsvisning, katalog, fordeler). Vi mangler fortsatt IAM-forankring, full CRUD for tiltak/tilskudd og opprydding i Figma-komponentene før admin-UI kan tas i bruk av redaktører i staging/prod.
