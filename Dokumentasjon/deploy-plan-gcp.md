...
- Kjør ny staging-deploy med oppdatert image (`gcloud builds submit ... _DEPLOY=true`) og bekreft at `/config/app.json` og `/api/*` svarer 200 (smoke via `curl`/`npm run test:smoke`). (Neste agent)
- Når API-ene svarer: smoke mot staging (`npm run test:smoke`), manuell UI-sjekk når CDN er koblet til, og verifiser gul liste/andre endpoints med gyldige secrets. (Neste agent)
