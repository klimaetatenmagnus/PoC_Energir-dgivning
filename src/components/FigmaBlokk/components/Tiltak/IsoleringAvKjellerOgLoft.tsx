...
interface IsoleringAvKjellerOgLoftProps {
  onBack?: () => void;
  buildingType?: string;
  buildingData?: any;
}

export const IsoleringAvKjellerOgLoft: React.FC<IsoleringAvKjellerOgLoftProps> = ({ onBack, buildingType, buildingData }) => {
  ...
  useEffect(() => {
    const fetchStotteordninger = async () => {
      try {
        ...
        const url = `http://localhost:3001/api/stotteordninger-live?gulliste=false&tiltak=etterisolering_kjeller_loft&bygningstype=${mappedType}`;
      ...
      >
        ...
        {(() => {
          ...
          if (byggeaar && buildingCategory) {
            ...
            console.log('IsoleringAvKjellerOgLoft - Energy calculation:', {
              byggeaar,
              tek,
              tekKey,
              buildingCategory,
              bruksareal,
              hasData: !!ENERGY_SAVINGS_DATA[tekKey],
              hasBuildingCategory: !!(ENERGY_SAVINGS_DATA[tekKey] && ENERGY_SAVINGS_DATA[tekKey][buildingCategory]),
              etteriso_takloft: ENERGY_SAVINGS_DATA[tekKey]?.[buildingCategory]?.["etteriso_takloft"]
            });
