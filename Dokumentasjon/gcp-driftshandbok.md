...
- Dokumenter eventuelle Excel/CSV-oppdateringer i `Dokumentasjon/` og logg endringer i endringsloggen.
...
8. **Logg endringen** i endringsloggen (§ 9).
...
| ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| 2026-02-15 | **Git-historikk renset (steg 4):** Fjernet alle sensitive filer fra hele git-historikken via `git filter-branch`: `data/raw/Matrikkel 2023.csv`, `data/raw/enova-energimerker-oslo.csv`, `data/raw/Input til modellen-Tabell 1.csv`, `data/raw/energimerke-grenser.json`, `.gcloud/`, `.gcloud-config/`, `.config/gcloud/`, `Matrikkel 2023.csv` (rotnivå). Repo-størrelse redusert fra 142 MB til 104 MB. Force-pushet til GitHub. | Claude |
| 2026-02-15 | **Matrikkel CSV migrert til GCS (steg 3):** (1) `csvService.ts` laster nå fra `gs://energinokkelen-data/matrikkel/Matrikkel 2023.csv` via `@google-cloud/storage` når `DATA_BUCKET` er satt, med fallback til lokal fil for utvikling. (2) `DATA_BUCKET=energinokkelen-data` lagt til i `cloudrun.yaml` for `building-info-service`. (3) `data/`-kopiering fjernet fra Dockerfile. (4) Matrikkel CSV fjernet fra git-tracking. (5) Oppdateringsrutine dokumentert i § 5.7. | Claude |
